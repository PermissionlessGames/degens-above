...
contract TestDegensAbove is Test {
    ...
    function test_nextRace() public {
        ...
        assertEq(
            game.RaceStartedAt(expectedRaceID),
            block.number + game.BettingPhaseBlocks(),
            "Race should start (and betting end) after BettingPhaseBlocks"
        );
    }
    ...
    function test_placeBet() public {
        ...
        assertEq(
            game.RacePot(raceID),
            initialRacePot + expectedPotIncrease,
            "Race pot should increase by bet amount minus rake"
        );
        assertEq(
            game.RaceBalance(raceID),
            initialRaceBalance + expectedPotIncrease,
            "Race balance should increase by bet amount minus rake"
        );
        assertEq(
            game.RacePlayerTotalBets(raceID, player1),
            initialPlayerTotalBets + 1,
            "Player total bets should increase by 1"
        );
        assertEq(
            game.RacePlayerChariotBets(raceID, player1, chariotID),
            initialPlayerChariotBet + 1,
            "Player chariot bet should increase by 1"
        );
    ...
    function test_placeBet_multiple() public {
...

        {
            ...
            assertEq(
                game.RacePot(raceID),
                initialRacePot + expectedPotIncrease,
                "Race pot should increase by total bet amount minus rake"
            );
            assertEq(
                game.RaceBalance(raceID),
                initialRaceBalance + expectedPotIncrease,
                "Race balance should increase by total bet amount minus rake"
            );
            assertEq(
                game.RacePlayerTotalBets(raceID, player1),
                initialPlayerTotalBets + numBets,
                "Player total bets should increase by number of bets"
            );
            assertEq(
                game.RacePlayerChariotBets(raceID, player1, chariotID),
                initialPlayerChariotBet + numBets,
                "Player chariot bet should increase by number of bets"
            );
            assertEq(
                player1.balance,
                initialPlayerBalance - (numBets * game.BetSize()),
                "Player balance should decrease by total bet amount (remainder returned)"
            );
        }
    }
    ...
    function test_increasePot() public {
        ...
        assertEq(game.RacePot(raceID), initialRacePot + addAmount, "Race pot should increase by the added amount");
        assertEq(
            game.RaceBalance(raceID), initialRaceBalance + addAmount, "Race balance should increase by the added amount"
        );
        assertEq(
            player1.balance, initialPlayerBalance - addAmount, "Player balance should decrease by the added amount"
        );
    }
    ...
    function test_increasePot_zero_amount() public {
        ...
        assertEq(game.RacePot(raceID), initialRacePot, "Race pot should remain unchanged with zero amount");
        assertEq(game.RaceBalance(raceID), initialRaceBalance, "Race balance should remain unchanged with zero amount");
    }
    ...
    function test_placeBet_rake_with_no_next_race() public {
        ...
        assertEq(game.RacePot(nextRaceID), expectedRake, "Next race pot should be set to rake amount (1/8 of bet)");
    }
    ...
    function test_increasePot_future_race() public {
        ...
        assertEq(game.RacePot(futureRaceID), addAmount, "Future race pot should be set to the added amount");
        assertEq(game.RaceBalance(futureRaceID), addAmount, "Future race balance should be set to the added amount");
        assertEq(
            player1.balance, initialPlayerBalance - addAmount, "Player balance should decrease by the added amount"
        );
    }
    ...
    function test_placeBet_rake_emits_pot_increased_event() public {
        ...
        assertEq(game.RacePot(nextRaceID), rake, "Next race pot should be increased by the rake amount");
    }
    ...
    function test_position_tracking() public {
        ...
        for (uint256 i = 0; i < 16; i++) {
            assertEq(game.RaceChariotPositionSnapshot(raceID, i), 0, "Initial position should be 0");
        }
        ...
        for (uint256 i = 0; i < 16; i++) {
            ...
            assertEq(
                game.getChariotPosition(raceID, i), expectedPosition, "Position calculation during race is incorrect"
            );
        }
        ...
        for (uint256 i = 0; i < 16; i++) {
            if (game.RaceChariotPositionSnapshot(raceID, i) == maxDistance) {
                assertTrue(game.RaceWinners(raceID, i), "Chariot at max distance should be a winner");
            } else {
                assertFalse(game.RaceWinners(raceID, i), "Chariot not at max distance should not be a winner");
            }
        }
    }
}
